			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="sessionCode" title="${message(code: 'session.sessionCode.label', default: 'Session Code')}" />
				...
				<g:each in="${sessionInstanceList}" status="i" var="sessionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${sessionInstance.id}">${fieldValue(bean: sessionInstance, field: "sessionCode")}</g:link></td>
					
						<td>${fieldValue(bean: sessionInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: sessionInstance, field: "course")}</td>
